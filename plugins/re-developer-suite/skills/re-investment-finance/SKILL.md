...
```
1. Sàng lọc deal            → re-investment-screening
2. Báo cáo đầu tư sơ bộ      → re-preliminary-investment-report
3. Feasibility Study (FS)    → re-feasibility-study   (spec + Excel)
4. Báo cáo đầu tư đầy đủ/IC  → re-full-investment-report
5. Cấu trúc deal + LOI       → deal-structuring-advisor
6. Điều phối DD              → dd-coordinator
   (QC mọi output)           → re-investment-verification-rules
```
...
Do not use for: legal status / clause review một stream (→ `RE-Legal`); area study / comps độc lập (→ `RE-Market-Research`); concept / product-mix / chỉ tiêu quy hoạch (→ `RE-Project-Design`); quyết định đa phòng cấp executive (→ `RE-HQ`).

## Specialist input (pull rule)

- **Pháp lý dự án** (đất / đầu tư / quy hoạch / điều kiện mở bán / chuyển nhượng) → kéo `licensing-expert` (+ `tvpl` để kiểm chứng hiệu lực).
- **Pháp lý giao dịch** (SPA/SHA/JVA, CP, change-of-control) → kéo `legal-counsel`.
- **Thị trường** (comps, cung-cầu, giá, absorption) → kéo `re-market-research` / `vn-re-research`.
- **Chỉ tiêu quy hoạch / product mix / GFA** cho pre-FS và FS → kéo `design-planning`.
...
0. `re-investment-operating-matrix` để chọn nhanh skill + specialist pull + template + verification theo giai đoạn (khi lane đã rõ).
1. `re-investment-screening` (nếu deal mới đầu vào)
2. skill theo giai đoạn lifecycle (preliminary → FS → full/IC → structuring → DD)
3. specialist pull (`licensing-expert`/`legal-counsel`/`re-market-research`/`vn-re-research`/`design-planning`) khi cần
4. `re-investment-verification-rules` trước khi chốt output chính thức
