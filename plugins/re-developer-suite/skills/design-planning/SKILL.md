...
6. Khi tính tuân thủ liên quan QCVN/luật, kiểm chứng hiệu lực văn bản qua `tvpl` (xem `../../references/tvpl-lookup-protocol.md`); khả năng phê duyệt pháp lý kéo `licensing-expert`.
...
Chỉ có DT đất + loại hình → giả định D/H/FAR theo QCVN. D×H ra FAR > FAR_max → cảnh báo, đề xuất giảm D hoặc H. Muốn vượt quy chuẩn → phân tích rủi ro pháp lý + PA xin điều chỉnh QH (kéo `licensing-expert`). Mixed-use → tính riêng từng khối, tổng hợp chung. Dự án ven biển/đảo → hành lang bảo vệ bờ biển; gần sân bay → tĩnh không hàng không.

## Cross-link

- `re-feasibility-study` / `re-preliminary-investment-report` dùng GFA/NSA/product mix từ skill này làm input doanh thu–chi phí.
- `licensing-expert` cho khả năng phê duyệt / điều chỉnh quy hoạch.
