...
Không dùng cho: phân tích khả thi chi tiết (→ `re-inv-preliminary-report` / `re-inv-feasibility-study`); thẩm định sâu (→ `re-inv-dd-coordinator`).
...
- **Pháp lý sơ bộ**: có red flag rõ ràng (đất chưa sạch, chưa có chủ trương, tranh chấp, condotel/officetel hạn chế sổ) — nếu cần, kéo `re-legal-licensing` 1 câu hỏi.
- **Thị trường sơ bộ**: giá vào so với mặt bằng khu vực hợp lý không (kéo `re-rnd` nếu cần 1 con số).
...
Nếu kết luận là **Proceed**: tạo deal dossier `deals/<deal-id>/_dossier.md` từ `../../templates/deal-dossier.md` (nếu chưa có) và ghi kết quả screening vào bảng lifecycle.
