...
- **Regulatory feasibility / project-side legal constraints** → gọi `re-legal-licensing`
...
- **Corporate legal / governance / transaction document implications** → gọi `re-legal-counsel`
...
- **Cần polish legal recommendation bằng tiếng Việt** → sau khi legal specialist hoàn tất, có thể yêu cầu `re-legal-writing` trong `RE-Legal`.
...
Với legal stream:
- gọi `re-legal-licensing` khi câu hỏi là project legal feasibility / regulatory constraints;
- gọi `re-legal-counsel` khi câu hỏi là governance / corporate / transaction document implications.
...
Khi đã chốt cấu trúc, gắn với kết quả `re-inv-feasibility-study` để dựng đề nghị giao dịch:
...
- nêu rõ phần **ràng buộc / không ràng buộc** của LOI và kéo `re-legal-counsel` rà ngôn ngữ pháp lý trước khi phát hành.
...
- Legal review owner (`re-legal-counsel`): ...
```
...
1. Biến `re-inv-deal-structuring` thành legal memo writer thay vì coordinator.
...
- [ ] Project legal feasibility đã map sang `re-legal-licensing` khi cần
- [ ] Corporate / governance / transaction legal implications đã map sang `re-legal-counsel` khi cần
