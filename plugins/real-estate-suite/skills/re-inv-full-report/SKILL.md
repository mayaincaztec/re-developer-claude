...
Báo cáo đầu tư chuyên sâu cuối cùng trong `RE-Investment-Finance`, gắn số từ FS, dùng để trình quyết định đầu tư (Investment Committee). Là bản sâu hơn của `re-inv-preliminary-report`, với phân tích và bằng chứng đầy đủ.
...
Không dùng cho: đánh giá sơ bộ (→ `re-inv-preliminary-report`); chỉ dựng model (→ `re-inv-feasibility-study`).
