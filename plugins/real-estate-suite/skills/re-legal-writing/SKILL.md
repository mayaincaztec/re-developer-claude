...
- đóng vai companion skill cho `re-legal-licensing` hoặc `re-legal-counsel`.
...
- thay thế specialist analysis của `re-legal-licensing` hoặc `re-legal-counsel`.

## Định tuyến

### 1. Dùng cùng `re-legal-licensing` khi:
...
### 2. Dùng cùng `re-legal-counsel` khi:
...
6. Nếu nội dung liên quan thủ tục dự án / cơ quan / điều kiện pháp lý dự án, áp dụng cùng workflow của `re-legal-licensing`.
7. Nếu nội dung liên quan hợp đồng / risk allocation / đàm phán / tranh chấp, áp dụng cùng workflow của `re-legal-counsel`.
...
6. Không phân định rõ phần kết luận thuộc `re-legal-licensing` hay `re-legal-counsel`.
...
- [ ] Nếu là licensing / procedural memo: đã kiểm tra thẩm quyền, hiệu lực văn bản, địa phương và bước thủ tục theo `re-legal-licensing`
- [ ] Nếu là counsel / transaction memo: đã xác định đúng client-side, risk allocation, fallback và deliverable theo `re-legal-counsel`
