> **Lưu ý kiểm chứng:** Mọi quy định, thuế suất, ngưỡng, thủ tục và cơ quan có thẩm quyền phải được kiểm tra lại bằng nguồn chính thức còn hiệu lực tại ngày sử dụng.

# Bộ Từ Điển Viết Tắt — Đồng Bộ Magic-Renamer

## TỪ ĐIỂN LOẠI VĂN BẢN (doc_type_mapping)

| Loại văn bản (viết thường) | Viết tắt |
|---|---|
| báo cáo | BC |
| biên bản | BB |
| chỉ thị | CT |
| công văn | CV |
| giấy phép xây dựng | GPXD |
| kế hoạch | KH |
| luật | Luat |
| nghị định | ND |
| nghị quyết | NQ |
| quyết định | QD |
| thông báo | TB |
| thông tư | TT |
| tờ trình | TTr |

---

## TỪ ĐIỂN TỈNH/THÀNH PHỐ (province_mapping)

| Tỉnh/Thành phố | Viết tắt |
|---|---|
| an giang | AGI |
| bà rịa - vũng tàu / bà rịa vũng tàu / bà rịa – vũng tàu | VTU |
| bạc liêu | BLU |
| bắc giang | BGI |
| bắc kạn | BKN |
| bắc ninh | BNI |
| bến tre | BTE |
| bình dương | BDU |
| bình định | BDI |
| bình phước | BPC |
| bình thuận | BTN |
| cà mau | CMA |
| cao bằng | CBA |
| cần thơ | CTH |
| đà nẵng | DNG |
| đắk lắk | DLA |
| đắk nông | DNO |
| điện biên | DBI |
| đồng nai | DNA |
| đồng tháp | DTH |
| gia lai | GLA |
| hà giang | HGI |
| hà nam | HNM |
| hà nội | HNO |
| hà tĩnh | HTI |
| hải dương | HDU |
| hải phòng | HPH |
| hậu giang | HAG |
| hòa bình | HBI |
| hưng yên | HYE |
| khánh hòa | KHO |
| kiên giang | KGI |
| kon tum | KTM |
| lai châu | LCH |
| lạng sơn | LSO |
| lào cai | LCA |
| lâm đồng | LDO |
| long an | LAN |
| nam định | NDI |
| nghệ an | NAN |
| ninh bình | NBI |
| ninh thuận | NTH |
| phú thọ | PTH |
| phú yên | PYE |
| quảng bình | QBI |
| quảng nam | QNA |
| quảng ngãi | QNG |
| quảng ninh | QNI |
| quảng trị | QTR |
| sóc trăng | STR |
| sơn la | SLA |
| tây ninh | TNI |
| thái bình | TBH |
| thái nguyên | TNG |
| thanh hóa | THO |
| thừa thiên huế / thừa thiên - huế / thành phố huế / huế | HUE |
| tiền giang | TGI |
| trà vinh | TVI |
| tuyên quang | TQU |
| vĩnh long | VLO |
| vĩnh phúc | VPH |
| yên bái | YBA |
| hồ chí minh / tp. hồ chí minh / tp hồ chí minh / thành phố hồ chí minh | HCM |

---

## TỪ ĐIỂN CƠ QUAN TRUNG ƯƠNG (agency_mapping)

| Cơ quan (viết thường) | Viết tắt |
|---|---|
| bộ công an | BCA |
| bộ công thương | BCT |
| bộ giáo dục và đào tạo | BGDDT |
| bộ giao thông vận tải | BGTVT |
| bộ kế hoạch và đầu tư | BKHDT |
| bộ khoa học và công nghệ | BKHCN |
| bộ lao động thương binh và xã hội | BLDTBXH |
| bộ ngoại giao | BNG |
| bộ nông nghiệp và môi trường | BNNMT |
| bộ nông nghiệp và phát triển nông thôn | BNNPTNT |
| bộ nội vụ | BNV |
| bộ quốc phòng | BQP |
| bộ tài chính | BTC |
| bộ tài nguyên và môi trường | BTNMT |
| bộ thông tin và truyền thông | BTTTT |
| bộ tư pháp | BTP |
| bộ văn hóa thể thao và du lịch | BVHTTDL |
| bộ xây dựng | BXD |
| bộ y tế | BYT |
| chính phủ | CP |
| hội đồng nhân dân / hdnd / hđnd | HDND |
| hội đồng thẩm định giá đất | HDTDGD |
| quốc hội | QH |
| sở công thương | SCT |
| sở giao thông vận tải | SGTVT |
| sở giáo dục và đào tạo | SGDDT |
| sở kế hoạch và đầu tư | SKHDT |
| sở khoa học và công nghệ | SKHCN |
| sở lao động thương binh và xã hội | SLDTBXH |
| sở nội vụ | SNV |
| sở nông nghiệp và môi trường | SNNMT |
| sở nông nghiệp và phát triển nông thôn | SNNPTNT |
| sở quy hoạch kiến trúc | SQHKT |
| sở tài chính | STC |
| sở tài nguyên và môi trường | STNMT |
| sở thông tin và truyền thông | STTTT |
| sở tư pháp | STP |
| sở văn hóa thể thao và du lịch | SVHTTDL |
| sở xây dựng | SXD |
| sở y tế | SYT |
| thanh tra chính phủ | TTCP |
| tổng cục quản lý đất đai | TCQLDD |
| ủy ban nhân dân / ubnd | UBND |
| văn phòng chính phủ | VPCP |

**Quy tắc ghép cơ quan địa phương:**
- Cơ quan cấp Quận/Huyện: `[Viết tắt CQ]` (KHÔNG ghép tỉnh)
  - Ví dụ: `UBND huyện Nhà Bè` → `UBND Nha Be`
- Cơ quan cấp Sở tỉnh: `[Viết tắt Sở] [Viết tắt Tỉnh]`
  - Ví dụ: `Sở Xây dựng TP. HCM` → `SXD HCM`

---

## TỪ ĐIỂN TRÍCH YẾU (summary_mapping)

| Cụm từ gốc | Thay thế |
|---|---|
| bất động sản | BDS |
| báo cáo | BC |
| biên bản | BB |
| bồi thường | BT |
| bộ công thương | BCT |
| bộ giao thông vận tải | BGTVT |
| bộ kế hoạch và đầu tư | BKHDT |
| bộ tài chính | BTC |
| bộ tài nguyên và môi trường | BTNMT |
| bộ xây dựng | BXD |
| chính phủ | CP |
| chủ đầu tư | CDT |
| chủ tịch | CT |
| chủ trương đầu tư | CTDT |
| chuyển mục đích | CMD |
| cơ chế, chính sách | co che |
| cơ quan | CQ |
| cơ sở dữ liệu | CSDL |
| cộng đồng dân cư | CDDC |
| công ty cổ phần | CTCP |
| công văn | CV |
| của chính phủ | *(bỏ)* |
| của quốc hội | *(bỏ)* |
| điều chỉnh cục bộ | DCCB |
| dự thảo | *(bỏ)* |
| đô thị và nông thôn | DT&NT |
| đơn giá đất | don gia dat |
| giải phóng mặt bằng | GPMB |
| giấy chứng nhận | GCN |
| giấy phép xây dựng | GPXD |
| hạ tầng kỹ thuật | HTKT |
| hình thành trong tương lai | HTTTL |
| hồ sơ | HS |
| hồ chí minh | HCM |
| hỗ trợ | HT |
| hội đồng nhân dân | HDND |
| kế hoạch | KH |
| kết luận | KL |
| kết luận thanh tra | KLTT |
| kết quả | KQ |
| khó khăn, vướng mắc | vuong mac |
| khu dân cư | KDC |
| khu du lịch | KDL |
| khu đô thị | KDT |
| khu nhà ở | KNO |
| kinh doanh | KD |
| luật | Luat |
| một số điều | *(bỏ)* |
| một số điều của | *(bỏ)* |
| nghĩa vụ tài chính | NVTC |
| nghị định | ND |
| nghị quyết | NQ |
| nhà đầu tư | NDT |
| nhà ở xã hội | NOXH |
| pháp luật | PL |
| phương án | PA |
| phương án kiến trúc | PAKT |
| quy hoạch | QH |
| quy hoạch chi tiết | QHCT |
| quy hoạch chung | QHC |
| quy hoạch phân khu | QHPK |
| quyền sử dụng đất | QSDD |
| sửa đổi, bổ sung / sửa đổi bổ sung | SDBS |
| sử dụng đất | SDD |
| tái định cư | TDC |
| thành phố | TP |
| tháo gỡ, xử lý | thao go |
| thẩm định | tham dinh |
| thông báo | TB |
| thông tư | TT |
| thương mại dịch vụ | TMDV |
| tổng mặt bằng | TMB |
| trung tâm hành chính | TTHC |
| ủy ban nhân dân | UBND |
| văn bản | VB |
| văn phòng | VP |

---

## Lưu ý tra cứu

1. So sánh không phân biệt chữ hoa/thường.
2. Tra cứu từ dài trước (tránh thay nhầm cụm từ ngắn hơn nằm trong cụm từ dài hơn).
3. Sau khi thay thế từ điển, loại bỏ dấu tiếng Việt toàn bộ trích yếu.
4. Các cụm từ có ký hiệu `*(bỏ)*` → xóa hẳn khỏi trích yếu.
